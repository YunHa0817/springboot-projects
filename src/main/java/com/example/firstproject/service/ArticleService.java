...
public class ArticleService {
    ...
    public Article create(ArticleForm dto) {
        Article article = dto.toEntity();   //dto객체를 엔티티로 변환

        if (article.getId() != null) return null;
        return articleRepository.save(article); //article 엔티티 객체를 DB에 저장
    }
    ...
    public Article update(Long id, ArticleForm dto) {
        ...
        target.patch(article);  //기존 데이터에 새 데이터 추가
    ...
    public Article delete(Long id) {
        ...
        //2. 잘못된 요청 처리하기
        if (target == null) return null;    //응답은 컨트롤러가 하므로 여기서는 null 반환
    ...
    @Transactional  //아래의 메서드를 하나의 트랜잭션으로 묶임
